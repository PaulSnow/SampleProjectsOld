...
public class ChipApp {
	...
	public synchronized void update(int thread, int approved, int denied){
	...
	public void run() {		
		...
		try {
			...
			System.out.println("Processing " + jobs.size() + " tests");
				...
				jobcnt = 1;
			{
				...
				long sec = ((long)dt) / 1000;
				long lms = ((long)dt) - (sec * 1000);
				String ms = lms < 100 ? lms < 10 ? "00" + lms : "0" + lms : ""
						+ lms;
				System.out.println("\nDone.  A Job is processed every: " + sec + "."
						+ ms + " seconds");
				System.out.printf(" A Job is processed every %.12f seconds.\n\n",dt/1000);
